...
export default defineComponent({
  components: {TodoForm}, setup() {
    ...
    const handleSubmit = async (newItem) => {
      await todoStore.addTodo(newItem)
        .then(() => {
          Swal.fire({
            icon: "success",
    ...
    const handleDeleteComplete = async () => {
      if (todoStore.items.length === 0) {
        await Swal.fire({
          icon: "info", title: "No items found!", showConfirmButton: false, timer: 1500,
        });
        return;
      }
      await todoStore.deleteComplete()
        .then((res) => {
          if (res.deletedCount > 0) {
            ...
            Swal.fire({
              icon: "info", title: "No items to be deleted!", showConfirmButton: false, timer: 1500,
            });
            return;
          }
        })
    ...
    return {
      isDialogOpen, formData, handleOpenForm, handleSubmit, handleDeleteComplete,
    };
  }
})
</script>
